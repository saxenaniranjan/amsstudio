...
def _detect_dimension(query: str, df: pd.DataFrame) -> str | None:
    q = query.lower()
    explicit_patterns = [
        (r"\b(team|by team|per team|team wise)\b", "team"),
        (r"\b(category|by category|per category)\b", "category_derived"),
        (r"\b(business function|function wise|by function)\b", "business_function_derived"),
        (r"\b(priority|by priority|per priority)\b", "priority"),
        (r"\b(status|by status|per status)\b", "status"),
        (r"\b(by service|per service|service wise)\b", "service"),
    ]
    for pattern, value in explicit_patterns:
        if re.search(pattern, q) and value in df.columns:
            return value
...
def _aggregate(
    ...
    del metric_source  # metric_name fully determines aggregation here.
    ...
    if dimension == "created_at":
        ...
        if working.empty:
            return pd.DataFrame()
        working["period"] = working["created_at"].dt.to_period(granularity).dt.to_timestamp()
        group_col = "period"
    else:
    ...
    if group_col is None:
        if metric_name == "ticket_count":
            result = pd.DataFrame([{metric_name: int(len(working))}])
        elif metric_name == "avg_mttr_hours":
            result = pd.DataFrame([{metric_name: float(working["mttr_hours"].mean())}])
        elif metric_name == "breach_rate":
            result = pd.DataFrame([{metric_name: float(working["is_sla_breached"].mean())}])
        elif metric_name == "breached_tickets":
            result = pd.DataFrame([{metric_name: int(working["is_sla_breached"].sum())}])
        elif metric_name == "open_tickets":
            result = pd.DataFrame([{metric_name: int(working["is_open"].sum())}])
        elif metric_name == "team_performance_index":
            result = pd.DataFrame([{metric_name: float(working["team_performance_index"].mean())}])
        else:
            result = pd.DataFrame([{metric_name: int(len(working))}])
    else:
        result = _metric_agg(working)
